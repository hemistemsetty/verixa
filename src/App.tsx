...
const MainLayout: React.FC = () => {
  ...
  // New Verixa auth guard: protected pages are never rendered without a Supabase session.
  useEffect(() => {
    if (!isAuthenticated && PROTECTED_PAGES.has(String(currentPage))) {
  ...
  const hideSidebarPages = [
    'landing', 'login', 'signup', 'verify-email', 'about', 'privacy', 'terms', 'help',
    'contact', 'ai-architecture', ...(!isAuthenticated ? ['ai-dashboard', 'ai_dashboard'] : []),
  ];
  ...
  const renderPage = () => {
    switch (currentPage as string) {
      ...
      case 'home': return <HomeFeedPage onOpenCreatePost={() => setIsCreatePostOpen(true)} />;
      ...
      case 'ai-dashboard':
      case 'ai_dashboard': return isAuthenticated ? <AIDashboardPage /> : <AIArchitecturePreviewPage />;
      case 'ai-architecture': return <AIArchitecturePreviewPage />;
      case 'sentinel-ai':
      case 'sentinel_ai':
      case 'sentinel-chatbot': return <SentinelAIChatbotPage />;
  ...
  return (
    <div className="min-h-screen bg-[#050507] text-gray-100 flex flex-col font-sans selection:bg-purple-600 selection:text-white relative overflow-x-hidden">
      <div className="fixed top-[-100px] left-[-100px] w-[500px] h-[500px] bg-purple-600/10 rounded-full blur-[120px] pointer-events-none z-0" />
      <div className="fixed bottom-[-100px] right-[-100px] w-[600px] h-[600px] bg-blue-600/10 rounded-full blur-[120px] pointer-events-none z-0" />
      ...
      <div className="flex-1 flex max-w-[1600px] w-full mx-auto px-4 lg:px-6 pt-16 z-10 relative gap-6">
        ...
        <main className={`flex-1 min-w-0 ${currentPage === 'messages' ? 'h-[calc(100vh-64px)] py-2 overflow-hidden flex flex-col' : 'py-6'}`}>
          {renderPage()}
        </main>
      </div>
        ...
        <footer className="border-t border-white/5 bg-black/40 backdrop-blur-md py-6 px-6 text-[10px] text-gray-500 z-10 relative">
          <div className="max-w-6xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-2">
              <img src="/verixa-logo.jpg" alt="VERIXA Logo" className="w-6 h-6 rounded-lg object-cover shadow-[0_0_10px_rgba(37,99,235,0.4)] border border-purple-500/20" />
            ...
            <div className="flex flex-wrap items-center justify-center gap-4 text-gray-400">
              ...
              <button onClick={() => setCurrentPage('ai-architecture')} className="hover:text-blue-400 transition-colors">AI Architecture</button>
              ...
              <button onClick={() => setCurrentPage('help')} className="hover:text-blue-400 transition-colors">Help Center</button>
